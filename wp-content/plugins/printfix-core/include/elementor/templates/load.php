...
use \rr_ELEMENTOR\Templates\rr_Api;
use \rr_ELEMENTOR\Templates\rr_Templates as rr_init;
...
class rr_Load{
    ...
    public function load_template_views() {
        include rr_init::dir() . 'templates.php';
    }
   
    public static function get_library() {
        if ( is_null( self::$library_data ) ) {
            self::$library_data = new rr_Api();
        }
        return self::$library_data;
    }
 
    public function register_ajax_actions( Ajax $ajax ) {
        $ajax->register_ajax_action( 'get_rr-core_library_data', function( $data ) {
